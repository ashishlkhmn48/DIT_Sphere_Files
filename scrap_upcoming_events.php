...
// Get cURL resource
$curl = curl_init();
$url = "http://www.dituniversity.edu.in/happenings/events";
...
$date_array = array();
$date = array();
preg_match_all('/(\/event\/.*?)"/s',$match[0][0],$url_array,PREG_PATTERN_ORDER);
...
$query = new ParseQuery("UpcomingEvents");
...
for($i=$key-1 ; $i>=0 ; $i--){
    ...
    $news = new ParseObject("UpcomingEvents");
    ...
    try {
        ...
        while($row = mysqli_fetch_array($result)){
            $fields = array(
	        "to"=>$row["token"],
	        "data"=>array("heading_event"=>$heading[$i],"type"=>"events")
		        );
